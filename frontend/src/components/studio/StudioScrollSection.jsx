...
export default function StudioScrollSection() {
  ...
  return (
    <section 
      id="studio"
      ref={containerRef}
      className="relative w-full h-[400svh] sm:h-[440vh] md:h-[480vh] lg:h-[580vh] bg-[#080808] text-[#f4f3ef] border-t border-white/[0.08]"
    >
      {/* Sticky Cinematic Viewport Stage */}
      <div className="sticky top-0 h-screen h-[100dvh] h-[100svh] w-full flex flex-col justify-between p-3 sm:p-5 md:p-7 lg:p-8 xl:p-10 2xl:px-12 2xl:py-8 pt-[max(0.75rem,env(safe-area-inset-top))] pb-[max(0.75rem,env(safe-area-inset-bottom))] overflow-hidden select-none">
        
        {/* Top Editorial Bar — aligned to unified architectural container */}
        <div className="order-1 relative z-20 w-full max-w-6xl 2xl:max-w-[1680px] mx-auto flex items-center justify-between gap-2 sm:gap-4 border-b border-white/[0.08] pb-2 sm:pb-3.5 2xl:pb-4">
          <div className="min-w-0 pr-2">
            <div className="flex items-center space-x-1.5 sm:space-x-2 text-[9.5px] sm:text-xs 2xl:text-[13px] font-pencrow text-mirae-orange font-medium tracking-[0.18em] sm:tracking-[0.25em] uppercase mb-0.5 sm:mb-1">
              <Compass className="w-3 sm:w-3.5 2xl:w-4 h-3 sm:h-3.5 2xl:h-4 text-mirae-orange shrink-0" />
              <span className="truncate">THE ATELIER • SPATIAL SEQUENCE</span>
            </div>
            <h2 className="font-architectural text-xs xs:text-sm sm:text-xl md:text-2xl lg:text-3xl xl:text-4xl 2xl:text-[2.65rem] font-bold text-white uppercase tracking-wider truncate">
              WHERE VISION TAKES FORM
            </h2>
          </div>

          {/* Dynamic Index & Progress Bar */}
          <div className="flex items-center space-x-2.5 sm:space-x-8 2xl:space-x-10 shrink-0">
            <div className="flex flex-col items-end">
              <span className="text-[9px] sm:text-xs 2xl:text-[13px] font-pencrow text-white/50 uppercase tracking-widest font-medium">
                SEQUENCE
              </span>
              <span className="text-xs sm:text-base 2xl:text-lg font-pencrow font-semibold text-white tracking-widest">
                {activeItem.num} <span className="text-white/30">/ 06</span>
              </span>
            </div>

            {/* Architectural Progress Meter (GPU scaleX animation via GSAP) */}
            <div className="w-12 sm:w-28 md:w-36 2xl:w-44 h-[2px] bg-white/10 relative overflow-hidden rounded-full">
              <div 
                ref={progressFillRef}
        ...
        <div className="order-2 lg:order-3 relative z-20 max-w-6xl 2xl:max-w-[1680px] mx-auto w-full pt-1.5 sm:pt-3 lg:pt-3.5 2xl:pt-4 border-b lg:border-b-0 lg:border-t border-white/[0.08] pb-1.5 sm:pb-3 lg:pb-0 flex flex-col md:flex-row items-start md:items-end justify-between gap-2 sm:gap-4">
          {/* Stacked Animated Captions (Driven synchronously by GSAP timeline) */}
          <div className="relative w-full max-w-2xl 2xl:max-w-4xl min-h-[102px] xs:min-h-[96px] sm:min-h-[92px] md:min-h-[105px] 2xl:min-h-[120px] overflow-visible">
            {studioImages.map((item, idx) => (
              ...
              >
                <div className="text-[9px] sm:text-xs 2xl:text-[13px] font-pencrow text-mirae-orange font-medium tracking-widest uppercase mb-0.5 sm:mb-1 truncate">
                  {item.category} • {item.subtitle}
                </div>
                <h3 className="font-architectural text-sm sm:text-xl md:text-2xl 2xl:text-3xl font-bold text-white uppercase tracking-wide leading-tight mt-0.5">
                  {item.title}
                </h3>
                <p className="text-[11px] sm:text-sm 2xl:text-base font-normal text-neutral-300 mt-0.5 sm:mt-1 2xl:mt-2 leading-relaxed font-pencrow line-clamp-2 sm:line-clamp-3 md:line-clamp-none max-w-3xl">
                  {item.description}
                </p>
              </div>
            ))}
          ...
          <div className="hidden lg:flex items-center space-x-2 text-[9.5px] sm:text-xs 2xl:text-[13px] font-pencrow text-white/50 uppercase tracking-widest font-medium shrink-0">
            <span>SCROLL TO ADVANCE</span>
            <ArrowRight className="w-3 sm:w-3.5 2xl:w-4 h-3 sm:h-3.5 2xl:h-4 text-mirae-orange" />
          </div>
        </div>

        {/* Central Display: Layered Image Canvas (order-3 on mobile, lg:order-2 on desktop) */}
        <div className="order-3 lg:order-2 relative z-10 flex-grow my-1 sm:my-2 md:my-3 lg:my-4 w-full max-w-6xl 2xl:max-w-[1680px] mx-auto flex items-center justify-center overflow-hidden">
          <div className="relative w-full h-full max-h-[38svh] sm:max-h-[44svh] md:max-h-[50vh] lg:max-h-[54vh] xl:max-h-[58vh] 2xl:max-h-[62vh] aspect-[4/3] sm:aspect-[16/10] lg:aspect-[16/9] overflow-hidden border border-white/[0.1] bg-[#0c0c0c] shadow-2xl">
            {studioImages.map((item, idx) => (
              <div
                key={item.id}
              ...
              >
                ...
                {/* Badge Tag */}
                <div className="absolute top-2.5 left-2.5 sm:top-4 sm:left-4 z-10 bg-black/70 backdrop-blur-md px-2 sm:px-2.5 2xl:px-3 py-1 2xl:py-1.5 border border-white/15 text-[8.5px] sm:text-[11px] 2xl:text-xs font-pencrow font-medium tracking-[0.2em] text-white/90 uppercase">
                  {item.tag}
                </div>
              </div>
            ))}
        ...
        {/* Mobile Advance Prompt (order-4 on mobile, hidden on desktop) */}
        <div className="order-4 flex lg:hidden items-center justify-between text-[9px] font-pencrow text-white/40 tracking-wider pt-1 border-t border-white/[0.04]">
          <span>STUDIO TRIPTYCH SEQUENCE</span>
          <span className="flex items-center space-x-1 text-white/60">
            <span>SCROLL</span>
            <ArrowRight className="w-2.5 h-2.5 text-mirae-orange" />
          </span>
        </div>
